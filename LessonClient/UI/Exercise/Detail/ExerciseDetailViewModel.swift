...
final class ExerciseDetailViewModel: ObservableObject {
    ...
    func saveOptions() async {
        ...
        let options: [ExerciseOptionUpdate] = currentOptions.map {
            return ExerciseOptionUpdate.textOption($0.lowercased())
        }
        ...
        do {
            let updated = try await ExerciseDataSource.shared.update(id: practice.id, practice: update)
    ...
    func delete() async -> Bool {
        ...
        errorMessage = nil
        do {
            try await ExerciseDataSource.shared.delete(id: practice.id)
            isDeleting = false
            return true
        } catch {
            isDeleting = false
